...
class EconomicSystem(System):
    def update(self, agent_graph: AgentGraph, **kwargs):
        ...
        income = torch.zeros(agent_graph.num_nodes(), device=self.device)

        if params.health_based_income:
            # Income is proportional to health for adults
            adult_health = agent_graph.ndata[AgentPropertyKeys.HEALTH][is_adult]
        ...
        current_wealth = agent_graph.ndata[AgentPropertyKeys.WEALTH]
        new_wealth = torch.clamp(current_wealth + income, min=0.0, max=1.0)
        agent_graph.ndata[AgentPropertyKeys.WEALTH] = new_wealth