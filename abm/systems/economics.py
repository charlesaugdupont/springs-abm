...
class EconomicSystem(System):
    ...
    configured) and the household's total cost of living (charged per
    member) are aggregated at the household level. The net change is
    applied to the shared balance, which is then re-broadcast to every
    member. This reflects money being shared within a household, and
    ...
    def update(self, agent_state: AgentState, **kwargs):
        ...
        # Cost of living is charged per household member, then pooled.
        household_cost = household_size * params.daily_cost_of_living
