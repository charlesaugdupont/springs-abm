...
class SVEIRModel(Model):
    ...
    def get_total_infections(self) -> int:
        # Summing both tracks for a total metric
        total = 0
        if "num_infections_rota" in self.graph.ndata:
            total += torch.sum(self.graph.ndata["num_infections_rota"]).item()
        if "num_infections_campy" in self.graph.ndata:
            total += torch.sum(self.graph.ndata["num_infections_campy"]).item()
        return total
    
    def get_proportion_infected_at_least_once(self) -> float:
        if "num_infections_rota" not in self.graph.ndata:
            return 0.0
        # Consider an agent infected if they had EITHER disease
        infected_mask = (self.graph.ndata["num_infections_rota"] > 0) | \
                        (self.graph.ndata["num_infections_campy"] > 0)
    ...
    def get_final_infection_counts(self) -> np.ndarray:
        if "num_infections_rota" not in self.graph.ndata: return np.array([])
        return (self.graph.ndata['num_infections_rota'] + self.graph.ndata['num_infections_campy']).cpu().numpy()
    ...
    def create_grid(self):
        ...
        if grid_params.method == "realistic_import":
            ...
            self.grid_environment.grid_shape = self.grid_tensor.shape
        else:
    ...
    def initialize_agent_properties(self):
        ...
        # --- ROTAVIRUS TRACK ---
        rota_prop = self.config.steering_parameters.rotavirus.initial_infected_proportion
        agent_properties["status_rota"] = self._initialize_agents_compartment(proportion=rota_prop)
        agent_properties["exposure_time_rota"] = torch.zeros(num_agents, dtype=torch.int)
    ...
    def _initialize_agents_compartment(self, proportion):
    ...
    def _initialize_agents_time_use(self):
        ...
        base_child[:, Activity.WORSHIP] = 0.0
        base_child[:, Activity.WATER] = 0.0
        base_child[:, Activity.SOCIAL] = 0.0
